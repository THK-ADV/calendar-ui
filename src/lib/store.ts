...
export const roomsAsChoiceOptions = derived(rooms, ($rooms) =>
  $rooms.map(roomToChoiceOption)
)

const teachingUnitToChoiceOption = (
...
const studyProgramToChoiceOption = (
...
export const selectedTeachingUnit: Writable<ChoiceOption | undefined> =
  writable()
export const selectedStudyProgram: Writable<ChoiceOption | undefined> =
  writable()
    ...
    const filteredStudyPrograms =
      $selectedTeachingUnit === undefined
        ...
        : $studyPrograms.filter(
            ({ teachingUnit }) => teachingUnit === $selectedTeachingUnit?.value
          )
...
const filteredSemesterPlan = derived(
  ...
  ([semesterPlanEvents, selectedTeachingUnit, selectedSemester]) => {
    ...
    for (const entry of semesterPlanEvents) {
      if (
        selectedTeachingUnit &&
        entry.teachingUnit === selectedTeachingUnit.value
      ) {
        if (!entry.semester.index) {
          ...
          if (
            ...
            entry.semester.index.includes(selectedSemester.value)
          ) {
            filteredEntries.push(entry)
          }
        }
...
export const filteredEvents = derived(
  ...
  ([
    ...
    return [
      ...
      ...$semesterPlanEvents.map((a) =>
        semesterPlanToFullCalendarEvent(a, $selectedSemester?.value)
      )
    ]
  }
)
...
export const selectedScheduleEvent: Writable<ScheduleEvent | undefined> =
  writable()

// Subscriptions

selectedScheduleEventId.subscribe((id) => {
  selectedScheduleEvent.set(
    get(scheduleEvents).find((scheduleEvent) => scheduleEvent.id === id)
  )
})
